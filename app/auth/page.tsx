...
function AuthContent({ workspaceId }: AuthContentProps) {
  ...
  const fetchExistingData = async () => {
    try {
      ...
      debugLog('Auth page session check:', { session });

      if (!session) {
        debugLog('No Supabase session, checking cookie');
        try {
          const cookieStr = sessionStorage.getItem('cookie');
          if (cookieStr) {
            session = JSON.parse(cookieStr);
            debugLog('Found session from cookie:', session);
          }
        } catch (err) {
          debugLog('Error parsing cookie:', err);
          return; // Don't proceed if we can't get a valid session
        }
      }

      if (!session?.user?.id) {
        debugLog('No valid user ID in session');
        return;
      }

      // First check if we have everything we need
      const [profileResult, workspacesResult, accessResult] = await Promise.all([
        supabase
          .from('user_profiles')
          .select('username')
          .eq('id', session.user.id)
      ...
      debugLog('User state check:', {
        hasProfile: !!profileResult.data,
        workspaceCount: workspacesResult.data?.length,
        hasAccess: !!accessResult.data
      });

      const hasProfile = profileResult.data && !profileResult.error;
      const hasWorkspace = workspacesResult.data && workspacesResult.data.length > 0;
      const hasAccess = accessResult.data && !accessResult.error;
      const isNewSignup = session.user.user_metadata?.is_new_signup === true;

      // Determine where to send the user
      if (!hasAccess) {
        debugLog('User needs access verification, redirecting to access page');
        router.push('/access');
        return;
      }

      if (!hasProfile || !hasWorkspace) {
        debugLog('User needs onboarding, redirecting to onboarding page');
        router.push('/onboarding');
        return;
      }

      if (hasProfile && hasWorkspace && !isNewSignup) {
        debugLog('User has completed setup, redirecting to platform');
        router.push('/platform');
        return;
      }

      // If we get here, let the user stay on the auth page
      debugLog('User can stay on auth page');

    } catch (error) {
      debugLog('Error in fetchExistingData:', error);
    }
  }
